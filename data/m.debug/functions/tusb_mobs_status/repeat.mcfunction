...
#表示モード：HP
    #bossbar作成/削除
    execute as @a[tag=m.view_HP] at @s if entity @e[tag=Mob,distance=..20] run function m.debug:tusb_mobs_status/bossbar_hp_add
    execute as @a[tag=m.view_HP] at @s unless entity @e[tag=Mob,distance=..20] run bossbar remove mars:tusb_status_hp
    execute as @a[tag=m.view_HP] at @s unless entity @e[tag=Mob,distance=..20] run tag @a remove m.bossbar_view_hp
    #だれが見るか
...
#表示モード：MP
    #bossbar作成/削除
    execute as @a[tag=m.view_MP] at @s if entity @e[tag=Mob,distance=..20] run function m.debug:tusb_mobs_status/bossbar_mp_add
    execute as @a[tag=m.view_MP] at @s unless entity @e[tag=Mob,distance=..20] run bossbar remove mars:tusb_status_mp
    execute as @a[tag=m.view_MP] at @s unless entity @e[tag=Mob,distance=..20] run tag @a remove m.bossbar_view_mp
    #だれが見るか
